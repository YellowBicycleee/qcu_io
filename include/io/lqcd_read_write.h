...
class GaugeWriter{
public:
    GaugeWriter (const int mpi_rank, const std::vector<int>& mpi_desc) : mpi_rank_(mpi_rank), mpi_desc_(mpi_desc) {}
    void write(std::string file_path, qcu::io::GaugeStorage<std::complex<Real_>>& gauge_out);
protected:
    const int mpi_rank_;
    const std::vector<int>& mpi_desc_;
};
template <typename Real_ = double>
class GaugeReader{
public:
    GaugeReader (const int mpi_rank, const std::vector<int>& mpi_desc) : mpi_rank_(mpi_rank), mpi_desc_(mpi_desc) {}
    void read(std::string file_path, qcu::io::GaugeStorage<std::complex<Real_>>& gauge_in);
protected:
    const int mpi_rank_;
    const std::vector<int>& mpi_desc_;
};
